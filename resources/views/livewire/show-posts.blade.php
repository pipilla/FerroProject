        ...
        <div>
            @foreach ($selectedTags as $tag)
                <button type="button" wire:click="quitarTag({{ $tag->id }})"
                    class='text-gray-900 border border-white hover:border-gray-200 bg-white focus:ring-4 focus:outline-none focus:ring-gray-300 rounded-full text-base px-5 py-2.5 text-center me-3 mb-3 font-bold'>{{ $tag->name }}
        ...
        @foreach ($posts as $post)
            <div class="bg-white rounded-2xl shadow-md p-6 space-y-4">
                ...
                <div class="flex justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($post->tags as $tag)
                            <button wire:click="anadirTag({{ $tag->id }})"
                                class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded hover:scale-105 transition-transform duration-200">
                                #{{ $tag->name }}
                            </button>
                        @endforeach
                    </div>
                    ...
                    <!-- Interacciones -->
                    <div class="flex space-4 text-gray-500">
